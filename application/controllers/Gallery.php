...
class Gallery extends CI_Controller
{
	...
	public function __construct()
	{
		// controller constructor
		parent::__construct();
		$this->load->model('Gallery_mdl', 'gallery_mdl');
	...
	public function add()
	{
		$this->form_validation->set_rules('gallery-name', 'gallery name', 'required|trim');

		if($this->form_validation->run() == false) {
			$this->template->set('title', 'Add Gallery');
			$this->template->render('template', 'add');
		} else {
			$file_path = "assets/img/gallery/";
			$folder = $this->gallery_mdl->get_next_id();
			...
			$form_data = array(
				'gallery_name' => $clean_post['gallery-name'],
				'thumbnail' => isset($file['thumbnail']) ? $file['thumbnail']:null,
			);
			// insert to database
			$result = $this->gallery_mdl->insert($form_data);
			if($result['status']) {
	...
	public function gallery()
	{
		...
		$this->template->render('template', 'gallery', $data);
